...
Dock またはタスクバーで独立したアプリとして扱われるデスクトップウィンドウで Pi Web を開くには、次を実行します：

```bash
pi-web app
```

App モードは Pi Web とともにインストールされるオプションの Electron ランタイムを使用し、独立したデスクトップアプリを追加でインストールしません。macOS では `Command+H` または Dock アイコンで Pi Web を隠したり復元したりできます。ウィンドウを閉じても Dock アプリは利用でき、`Command+Q` またはターミナルの `Ctrl+C` でウィンドウとサーバーの両方を終了します。

## 設定

ポートとホスト名では、コマンドラインオプションが対応する環境変数より優先されます。通常の Web モードでは、`--no-open` と `PI_WEB_NO_OPEN=1` のどちらを指定してもブラウザーの自動起動が無効になります。

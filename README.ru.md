...
Чтобы открыть Pi Web в настольном окне с отдельным значком в Dock или на панели задач, выполните:

```bash
pi-web app
```

Режим App использует необязательную среду выполнения Electron, установленную вместе с Pi Web, и не устанавливает отдельное настольное приложение. В macOS используйте `Command+H` или значок Dock, чтобы скрыть и восстановить Pi Web. Закрытие окна оставляет приложение доступным в Dock, а `Command+Q` или `Ctrl+C` в терминале завершает работу окна и сервера.

## Настройка

Для порта и имени хоста параметры командной строки имеют приоритет над соответствующими переменными окружения. В обычном веб-режиме и `--no-open`, и `PI_WEB_NO_OPEN=1` отключают автоматическое открытие браузера.

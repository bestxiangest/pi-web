...
如需在拥有独立 Dock 或任务栏身份的桌面窗口中打开 Pi Web，请运行：

```bash
pi-web app
```

App 模式使用随 Pi Web 安装的可选 Electron 运行时，不会额外安装独立桌面应用。在 macOS 上，可通过 `Command+H` 或 Dock 图标隐藏和恢复 Pi Web；关闭窗口后 Dock 应用仍然可用，执行 `Command+Q` 或在终端按 `Ctrl+C` 会同时退出窗口和服务。

## 配置

端口和主机名以命令行参数为准，优先于对应的环境变量。在常规网页模式下，`--no-open` 与 `PI_WEB_NO_OPEN=1` 中任意一个都会关闭自动打开浏览器。
